...
#   - Partner CPO: FR/CPO (the TenantPartner)
#
# Prerequisites:
...
# The CPO (FR/CPO) pushes sessions to our eMSP (FR/ZET).
# URL uses CPO's country_code/party_id per OCPI 2.2.1 spec.
# The session is stored with a tenantPartnerId linking it to FR/CPO.
# ===========================================================================

separator "1. PUT /sessions/FR/CPO/sess-001 — Create session 1 (ACTIVE charging session)"
run_curl 200 \
...
separator "2. PUT /sessions/FR/CPO/sess-002 — Create session 2 (COMPLETED with charging periods)"
run_curl 200 \
...
separator "3. GET /sessions/FR/CPO/sess-001 — Retrieve session 1"
run_curl 200 \
  "$RECEIVER_BASE_URL/FR/CPO/sess-001" \
  "${OCPI_HEADERS[@]}"

separator "4. GET /sessions/FR/CPO/sess-002 — Retrieve session 2"
run_curl 200 \
  "$RECEIVER_BASE_URL/FR/CPO/sess-002" \
  "${OCPI_HEADERS[@]}"

separator "5. GET /sessions/FR/CPO/nonexistent — Non-existent session (expect 404)"
run_curl 404 \
...
separator "6. PATCH /sessions/FR/CPO/sess-001 — Update kwh and add charging period"
run_curl 200 \
...
separator "7. GET /sessions/FR/CPO/sess-001 — Verify the patch"
run_curl 200 \
  "$RECEIVER_BASE_URL/FR/CPO/sess-001" \
  "${OCPI_HEADERS[@]}"

separator "8. PUT /sessions/FR/CPO/sess-001 — Replace session 1 (mark COMPLETED)"
run_curl 200 \
  -X PUT "$RECEIVER_BASE_URL/FR/CPO/sess-001" \
  "${OCPI_HEADERS[@]}" \
  -H "Content-Type: application/json" \
...
separator "9. GET /sessions/FR/CPO/sess-001 — Verify the replacement"
run_curl 200 \
  "$RECEIVER_BASE_URL/FR/CPO/sess-001" \
  "${OCPI_HEADERS[@]}"
...
separator "12. PATCH /sessions/FR/CPO/nonexistent — Patch non-existent (expect error)"
run_curl 404 \
