...
// this script is used to delete charging stations for a partner
...
async function countRemaining(): Promise<number> {
  const data = await gql(
    `
    query Count($where: ChargingStations_bool_exp!) {
  ...
  return data.ChargingStations_aggregate.aggregate.count as number;
}

async function fetchBatchIds(): Promise<string[]> {
  const data = await gql(
    `
    query GetBatch($where: ChargingStations_bool_exp!, $limit: Int!) {
      ChargingStations(
        where: $where
        limit: $limit
        order_by: { id: asc }
  ...
  return (data.ChargingStations as { id: string }[]).map((r) => r.id);
}
...
async function deleteBatch(ids: string[]): Promise<number> {
  const data = await gql(
    ...
    mutation DeleteBatch($ids: [String!]!) {
      delete_ChargingStations(where: { id: { _in: $ids } }) {
        affected_rows
      }
    }
  `,
  ...
  return data.delete_ChargingStations.affected_rows as number;
}
async function main() {
  console.log({
    HASURA_URL,
  ...
  console.log(`Starting ChargingStations count: ${remaining}`);
  ...
  while (remaining > 0) {
    ...
    console.log(
      `Batch ${batch}: deleting ${ids.length} ChargingStations (${ids[0]}..${ids[ids.length - 1]})`,
    );
