...
const TENANT_ID = Number(process.env.TENANT_ID ?? '8');
...
async function countRemaining(): Promise<number> {
  const data = await gql(
    `
    query Count($ti: Int!, $tp: Int!) {
      Tariffs_aggregate(
        where: {
          tenantId: { _eq: $ti }
          tenantPartnerId: { _eq: $tp }
        }
      ) {
        aggregate { count }
      }
  ...
  `,
    { ti: TENANT_ID, tp: TENANT_PARTNER_ID },
  );
  return data.Tariffs_aggregate.aggregate.count as number;
}

async function fetchBatchIds(): Promise<number[]> {
  const data = await gql(
    `
    query GetBatch($ti: Int!, $tp: Int!, $limit: Int!) {
      Tariffs(
        where: {
          tenantId: { _eq: $ti }
          tenantPartnerId: { _eq: $tp }
  ...
  `,
    { ti: TENANT_ID, tp: TENANT_PARTNER_ID, limit: BATCH_SIZE },
  );
  return (data.Tariffs as { id: number }[]).map((t) => t.id);
}
...
async function deleteBatch(ids: number[]): Promise<number> {
  const data = await gql(
    ...
    mutation DeleteBatch($ids: [Int!]!) {
      delete_Tariffs(where: { id: { _in: $ids } }) {
        affected_rows
      }
    }
  `,
  ...
  return data.delete_Tariffs.affected_rows as number;
}
...
async function main() {
  ...
  console.log(`Starting count: ${remaining}`);
  ...
  while (remaining > 0) {
    ...
    console.log(
      `Batch ${batch}: deleting ${ids.length} ids (${ids[0]}..${ids[ids.length - 1]})`,
    );
