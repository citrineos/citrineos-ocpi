...
const WHERE_FILTER = {
  ...
  // },
  ChargingStation: {
    Location: { ownerTenantPartnerId: { _eq: TENANT_PARTNER_ID } },
  },
};
...
async function countRemaining(): Promise<number> {
  const data = await gql(
    `
    query Count($where: Connectors_bool_exp!) {
  ...
  return data.Connectors_aggregate.aggregate.count as number;
}

async function fetchBatchIds(): Promise<number[]> {
  const data = await gql(
    `
    query GetBatch($where: Connectors_bool_exp!, $limit: Int!) {
      Connectors(
        where: $where
        limit: $limit
        order_by: { id: asc }
  ...
  return (data.Connectors as { id: number }[]).map((r) => r.id);
}
...
async function deleteBatch(ids: number[]): Promise<number> {
  const data = await gql(
    ...
    mutation DeleteBatch($ids: [Int!]!) {
      delete_Connectors(where: { id: { _in: $ids } }) {
        affected_rows
      }
    }
  `,
  ...
  return data.delete_Connectors.affected_rows as number;
}
...
async function main() {
  ...
  console.log(`Starting Connectors count: ${remaining}`);
  ...
  while (remaining > 0) {
    ...
    console.log(
      `Batch ${batch}: deleting ${ids.length} Connectors (${ids[0]}..${ids[ids.length - 1]})`,
    );
