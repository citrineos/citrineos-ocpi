...
// this script is used to delete locations for a partner
...
async function countRemaining(): Promise<number> {
  const data = await gql(
    `
    query Count($where: Locations_bool_exp!) {
  ...
  return data.Locations_aggregate.aggregate.count as number;
}

async function fetchBatchIds(): Promise<number[]> {
  const data = await gql(
    `
    query GetBatch($where: Locations_bool_exp!, $limit: Int!) {
      Locations(
        where: $where
        limit: $limit
        order_by: { id: asc }
  ...
  return (data.Locations as { id: number }[]).map((r) => r.id);
}
...
async function deleteBatch(ids: number[]): Promise<number> {
  const data = await gql(
    ...
    mutation DeleteBatch($ids: [Int!]!) {
      delete_Locations(where: { id: { _in: $ids } }) {
        affected_rows
      }
    }
  `,
  ...
  return data.delete_Locations.affected_rows as number;
}
...
async function main() {
  ...
  console.log(`Starting Locations count: ${remaining}`);
  ...
  while (remaining > 0) {
    ...
    console.log(
      `Batch ${batch}: deleting ${ids.length} Locations (${ids[0]}..${ids[ids.length - 1]})`,
    );
