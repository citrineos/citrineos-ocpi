...
export const CDR_DATASOURCE_SERVICE_TOKEN = new Token('CDR_DATASOURCE_SERVICE_TOKEN');
...
export class CdrsDatasource implements ICdrsDatasource {
  ...
  async getCdrs(cpoCountryCode: string, cpoPartyId: string, mspCountryCode: string, mspPartyId: string, dateFrom?: Date, dateTo?: Date, offset?: number, limit?: number): Promise<PaginatedResult<Cdr>> {
    ...
    result.data = await this.cdrMapper.mapTransactionsToCdrs(transactionResult.data);
