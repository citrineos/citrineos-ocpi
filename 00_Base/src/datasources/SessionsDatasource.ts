...
export const SESSION_DATASOURCE_SERVICE_TOKEN = new Token('SESSION_DATASOURCE_SERVICE_TOKEN');
...
export class SessionsDatasource implements ISessionsDatasource {
  ...
  async getSessions(cpoCountryCode: string, cpoPartyId: string, mspCountryCode: string, mspPartyId: string, dateFrom?: Date, dateTo?: Date, offset?: number, limit?: number): Promise<PaginatedResult<Session>> {
    const transactionResult = await this.transactionDatasource.getTransactions(cpoCountryCode, cpoPartyId, mspCountryCode, mspPartyId, dateFrom, dateTo, offset, limit);

    const result: PaginatedResult<Session> = new PaginatedResult<Session>();
    result.data = await this.sessionMapper.mapTransactionsToSessions(transactionResult.data);
