...
export function buildGenericServerErrorResponse<T>(data?: T, error?: Error): OcpiResponse<T> {
    const response: OcpiResponse<T> = new OcpiResponse<T>();
    response.status_code = OcpiResponseStatusCode.ServerGenericError;
...
export function buildGenericClientErrorResponse<T>(data?: T, error?: Error): OcpiResponse<T> {
    const response: OcpiResponse<T> = new OcpiResponse<T>();
...
export function buildUnknownLocationResponse<T>(data?: T, error?: NotFoundException): OcpiResponse<T> {
    const response: OcpiResponse<T> = buildGenericServerErrorResponse(data, error);
    response.status_code = OcpiResponseStatusCode.ClientUnknownLocation;
    return response;
}

export function buildUnknownSessionResponse<T>(data: T, error: NotFoundException): OcpiResponse<T> {
    const response: OcpiResponse<T> = buildGenericServerErrorResponse(data, error);
    response.status_code = OcpiResponseStatusCode.ClientGenericError;
    return response;
}
